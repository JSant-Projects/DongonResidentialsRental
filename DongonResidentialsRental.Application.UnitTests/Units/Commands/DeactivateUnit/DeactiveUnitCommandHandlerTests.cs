...
namespace DongonResidentialsRental.Application.UnitTests.Units.Commands.DeactivateUnit;
...
public sealed class DeactivateUnitCommandHandlerTests
{
    ...
    private readonly DeactivateUnitCommandHandler _handler;

    public DeactivateUnitCommandHandlerTests()
    {
        _handler = new DeactivateUnitCommandHandler(
            _unitRepository,
            _dateTimeProvider,
            _leaseRepository);
    }
    ...
    public async Task Handle_Should_Throw_NotFoundException_When_Unit_Does_Not_Exist()
    {
        ...
        var command = new DeactivateUnitCommand(unitId);
    ...
    public async Task Handle_Should_Throw_ConflictException_When_Unit_Has_Active_Lease()
    {
        ...
        var command = new DeactivateUnitCommand(unit.UnitId);
        ...
        await act.Should()
            ...
            .WithMessage($"{unit.UnitNumber} has an active lease. Deactivate operation is not allowed");
    }

    [Fact]
    public async Task Handle_Should_Deactivate_Unit_When_There_Is_No_Active_Lease()
    {
        // Arrange
        var today = new DateTime(2026, 3, 29, 10, 0, 0);
        var todayDateOnly = DateOnly.FromDateTime(today);
